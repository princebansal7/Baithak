...
const SpinHistory: React.FC<SpinHistoryProps> = ({ history, onClear }) => {
  ...
  return (
    <div className="rounded-2xl border border-violet-200/60 dark:border-white/10 overflow-hidden bg-white dark:bg-white/[0.02]">
      <div className="group flex items-center justify-between hover:bg-violet-50/70 dark:hover:bg-white/5 transition-all">
        <button
          onClick={() => setExpanded((e) => !e)}
          className="flex-1 flex items-center gap-2 px-4 py-3 text-sm font-semibold text-gray-600 dark:text-white/70 group-hover:text-gray-900 dark:group-hover:text-white transition-colors"
        ...
        {history.length > 0 && (
          <button
            onClick={onClear}
            className="px-3 py-3 text-gray-300 dark:text-white/20 hover:text-red-500 dark:hover:text-red-400 transition-colors duration-150"
                ...
                <AnimatePresence initial={false}>
                  {[...history].reverse().map((result) => (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: 10 }}
                      className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-violet-50/40 dark:bg-white/[0.03] hover:bg-violet-100/50 dark:hover:bg-white/[0.06] transition-all group"
                    >
                      <span
